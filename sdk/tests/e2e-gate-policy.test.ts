...
import { findGatePolicyPda, findMembershipPda } from "../src/pda/index.js";
...
const BRC_MINT = getCanonicalEcosystem(
...
function resolveGatePolicyBaseline(): {
  ...
  const applicationValue = process.env.BABYCOWANS_TEST_APPLICATION;

  const applicationAssetValue = process.env.BABYCOWANS_TEST_APPLICATION_ASSET;

  if (applicationValue && applicationAssetValue) {
    return {
      application: new PublicKey(applicationValue),

      applicationAsset: new PublicKey(applicationAssetValue),
    };
  }
  ...
  const applicationAssetMatch = output.match(
    /(?:^|\n).*?ApplicationAsset PDA:\s*([1-9A-HJ-NP-Za-km-z]{32,44})/u
  );

  if (!applicationMatch || !applicationAssetMatch) {
    throw new Error(`GATE_POLICY_BASELINE_RESOLUTION_FAILED\n${output}`);
  }

  console.log("GATE_POLICY_BASELINE_SELF_BOOTSTRAPPED=1");

  return {
    application: new PublicKey(applicationMatch[1]),

    applicationAsset: new PublicKey(applicationAssetMatch[1]),
  };
}

const { application, applicationAsset } = resolveGatePolicyBaseline();
...
const walletPath = "/tmp/babycowans-phase5-gate-wallet.json";
...
function tokenAccountAddress(mint: PublicKey, ownerPath: string): PublicKey {
  /*
   * spl-token-cli 5.5.0 requires --verbose when
   * --token and --owner are supplied to `address`.
   */
  const output = runCli("spl-token", [
    "address",
    "--verbose",
    "--token",
    mint.toBase58(),
  ...
  try {
    ...
    const matches = serialized.match(/[1-9A-HJ-NP-Za-km-z]{32,44}/g) ?? [];

    const candidate = matches.find((entry) => entry !== mint.toBase58());
...
async function fund(pubkey: PublicKey): Promise<void> {
...
const X26_EXACT_REJECTION_BY_LABEL = new Map<string, string>([
  ["Empty evidence", "GatePolicyNotSatisfied"],
]);
...
async function expectRejected(
  ...
  const expectedError = X26_EXACT_REJECTION_BY_LABEL.get(label);
  ...
  if (expectedError !== undefined) {
    ...
    console.log(`✓ ${label} rejected with ${expectedError}`);
...
console.log("===== PHASE 5 REAL GATE POLICY E2E =====");

await fund(wallet.publicKey);

const applicationInfo = await connection.getAccountInfo(application);

const applicationAssetInfo = await connection.getAccountInfo(applicationAsset);

expect(applicationInfo !== null, "Golden Application is missing");

expect(applicationAssetInfo !== null, "Golden ApplicationAsset is missing");

console.log("✓ BRC application baseline detected");
...
console.log("✓ Tier 3 membership created");
...
const brcTokenAccount = tokenAccountAddress(BRC_MINT, walletPath);
...
console.log("✓ Canonical BRC ownership fixture funded");
...
const nftMintPath = "/tmp/babycowans-phase5-nft-mint.json";

fs.writeFileSync(
  nftMintPath,
  JSON.stringify(Array.from(nftMintKeypair.secretKey))
);
...
const nftTokenAccount = tokenAccountAddress(nftMint, walletPath);
...
console.log("✓ NFT ownership fixture created");
...
const [gatePolicy] = findGatePolicyPda(PROGRAM_ID, applicationAsset);
 ...
 */
if ((await connection.getAccountInfo(gatePolicy, "confirmed")) !== null) {
  throw new Error("X38 oversized GatePolicy fixture is not fresh.");
}
 ...
 */
const x38RawTemplate = buildConfigureGatePolicyInstruction({
  programId: PROGRAM_ID,
  application,
  applicationAsset,
  gatePolicy,
...
const x38RawSevenConditionInstruction = new TransactionInstruction({
  programId: x38RawTemplate.programId,
  keys: x38RawTemplate.keys,
  data: Buffer.concat([
    x38RawTemplate.data.subarray(0, 8),
    x38ConditionCount,
    ...Array.from({ length: 7 }, () => x38EncodedCondition),
    Buffer.from([1]),
  ]),
});

try {
  await send([x38RawSevenConditionInstruction], [authority]);
} catch (error: unknown) {
...
if (
  ...
  if (
    ...
    x38TooManyConditionsLogs = fetchedLogs.map((entry) => String(entry));
  }
}
...
const x38TooManyConditionsIdentityMatched =
  x38TooManyConditionsDiagnostic.includes("TooManyGateConditions") ||
  x38TooManyConditionsDiagnostic.includes("Error Number: 6039") ||
  x38TooManyConditionsDiagnostic.includes("custom program error: 0x1797") ||
  x38TooManyConditionsDiagnostic.includes("custom program error: 0X1797");
...
console.log("X38_GATE_POLICY_MAX_CONDITIONS_PLUS_ONE=PASS");
...
console.log("✓ OR-of-AND Gate Policy configured");
...
await expectRejected("Zero HoldAmount threshold", () =>
  send(
    [
      buildConfigureGatePolicyInstruction({
        programId: PROGRAM_ID,
...
await expectRejected("Wrong HoldAmount mint", () =>
  send(
    [
      buildConfigureGatePolicyInstruction({
        programId: PROGRAM_ID,
...
await expectRejected("Zero MembershipTier threshold", () =>
  send(
    [
      buildConfigureGatePolicyInstruction({
        programId: PROGRAM_ID,
...
await expectRejected("Non-default MembershipTier mint", () =>
  send(
    [
      buildConfigureGatePolicyInstruction({
        programId: PROGRAM_ID,
...
await expectRejected("Duplicate condition kind in group", () =>
  send(
    [
      buildConfigureGatePolicyInstruction({
        programId: PROGRAM_ID,
...
await expectRejected("Non-contiguous GatePolicy groups", () =>
  send(
    [
      buildConfigureGatePolicyInstruction({
        programId: PROGRAM_ID,
...
const [x8ForeignGatePolicy] = findGatePolicyPda(
  PROGRAM_ID,
  x8ForeignApplicationAsset
);

const brcRawBalance = await connection.getTokenAccountBalance(
  brcTokenAccount,
  "confirmed"
);

const holdEqualityAmount = BigInt(brcRawBalance.value.amount);
      ...
      conditions: [
        {
          ...
          conditionType: 0,
          mint: BRC_MINT,
          minimumAmount: holdEqualityAmount,
          minimumTier: 0,
        },
      ],
    }),
...
expect(
  (await connection.getAccountInfo(x8ForeignGatePolicy)) !== null,
...
console.log("X8_7_HOLD_EQUALITY_BOUNDARY=PASS");
 ...
 */
await expectRejected("Cross-application GatePolicy substitution", () =>
  send(
    [
      buildVerifyGatePolicyInstruction({
        programId: PROGRAM_ID,
...
console.log("X8_7_CROSS_APPLICATION_GATE_POLICY_REJECTED=PASS");

expect(
  (await connection.getAccountInfo(gatePolicy)) !== null,
  "GatePolicy account missing"
);

console.log("✓ Gate Policy account exists");
...
await expectRejected("Empty evidence", () =>
  send(
    [
      buildVerifyGatePolicyInstruction({
        programId: PROGRAM_ID,
 ...
 * ----------------------------------------------------------
 */
await expectRejected("Foreign-program hold token evidence", () =>
  send(
    [
      buildVerifyGatePolicyInstruction({
        programId: PROGRAM_ID,
        ...
        gatePolicy,
        wallet: wallet.publicKey,
        holdTokenAccount: wallet.publicKey,
        membership,
      }),
    ],
    [wallet]
...
console.log("X8_6_FOREIGN_TOKEN_EVIDENCE_REJECTED=PASS");

await expectRejected("Membership-only evidence", () =>
  send(
    [
      buildVerifyGatePolicyInstruction({
        programId: PROGRAM_ID,
...
await expectRejected("Hold-only evidence", () =>
  send(
    [
      buildVerifyGatePolicyInstruction({
        programId: PROGRAM_ID,
...
await expectRejected("Wrong-member Membership evidence", () =>
  send(
    [
      buildVerifyGatePolicyInstruction({
        programId: PROGRAM_ID,
...
await expectRejected("Cross-application Membership evidence", () =>
  send(
    [
      buildVerifyGatePolicyInstruction({
        programId: PROGRAM_ID,
...
await expectRejected("Insufficient Membership tier", () =>
  send(
    [
      buildVerifyGatePolicyInstruction({
        programId: PROGRAM_ID,
...
await expectRejected("Suspended Membership evidence", () =>
  send(
    [
      buildVerifyGatePolicyInstruction({
        programId: PROGRAM_ID,
...
const x8ExpirationUnix = BigInt(Math.floor(Date.now() / 1000) + 4);
...
const x8ExpirationDeadlineMs = Number(x8ExpirationUnix) * 1000;

while (Date.now() <= x8ExpirationDeadlineMs + 1500) {
  await new Promise((resolve) => setTimeout(resolve, 250));
}

await expectRejected("Expired Membership evidence", () =>
  send(
    [
      buildVerifyGatePolicyInstruction({
        programId: PROGRAM_ID,
...
console.log("✓ AND branch succeeded: Hold BRC + Tier >= 3");
...
await expectRejected("Wrong-owner NFT evidence", () =>
  send(
    [
      buildVerifyGatePolicyInstruction({
        programId: PROGRAM_ID,
 ...
 */
await expectRejected("Wrong-mint NFT token-account substitution", () =>
  send(
    [
      buildVerifyGatePolicyInstruction({
        programId: PROGRAM_ID,
        ...
        gatePolicy,
        wallet: wallet.publicKey,
        nftTokenAccount: brcTokenAccount,
      }),
    ],
    [wallet]
  )
 ...
 */
await expectRejected("Foreign-program NFT evidence", () =>
  send(
    [
      buildVerifyGatePolicyInstruction({
        programId: PROGRAM_ID,
        ...
        gatePolicy,
        wallet: wallet.publicKey,
        nftTokenAccount: wallet.publicKey,
      }),
    ],
    [wallet]
  )
...
const x8ZeroNftBalance = await connection.getTokenAccountBalance(
  nftTokenAccount,
  "confirmed"
);
...
await expectRejected("Zero-balance NFT evidence", () =>
  send(
    [
      buildVerifyGatePolicyInstruction({
        programId: PROGRAM_ID,
...
const x8RestoredNftBalance = await connection.getTokenAccountBalance(
  nftTokenAccount,
  "confirmed"
);
...
console.log("✓ OR branch succeeded: NFT ownership");

console.log("✓ AND semantics validated");

console.log("✓ OR semantics validated");

console.log("PHASE5_GATE_POLICY_E2E_PASS");
